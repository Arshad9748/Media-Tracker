...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchList = async () => {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/list`, {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 text-white font-jakarta max-w-7xl mx-auto">

      {/* Welcome */}
      <h1 className="text-3xl sm:text-4xl md:text-5xl font-sniglet mb-2">Overview</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
            {recent.map(item => (
              <div key={item.id} className="rounded-xl overflow-hidden border border-slate-800 bg-slate-900 transition-transform duration-300 hover:scale-110">
                <img src={item.cover_image} alt={item.title} className="w-full h-44 sm:h-52 object-cover"/>
                <div className="p-2">
